...
def exclude_nodes(
    project_path: Path, net: nx.DiGraph, excl_pattern: str, incl_pattern: str
) -> Set[Path]:
    ...
    excl_nodes = [
        ...
        if n in excl_filepaths and attr["_type"] == "file"
    ]
    ...
    if incl_pattern:
        incl_edges = [
            ...
            if src in incl_filepaths or dst in incl_filepaths
        ]
        incl_net = nx.DiGraph()
        ...
        excl_nodes += [
            n
            for n, attr in net.nodes(data=True)
            if n not in incl_net.nodes() and attr["_type"] == "file"
        ]
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "path", type=str, help="Path to directory containing python project."
    )
    #
    parser.add_argument(
        "--dir-as",
    ...
    parser.add_argument(
        "--incl",
        type=str,
        help="Glob pattern for including files. Only files matching incl pattern AND files they import are shown.",
    )
    parser.add_argument(
        "--highlight",
        type=str,
        default=None,
        help="Name of EXTERNAL package. Files that import it will be highlighted via color.",
    )
    # layout components
    parser.add_argument(
        "--full-filepath",
    ...
    parser.add_argument(
        "--ignore-imports",
        action="store_true",
        help="Import links won't affect graph layout.",
    )
    parser.add_argument(
        "--show-class-bases",
        action="store_true",
    ...
    parser.add_argument(
        "--combine-links",
        action="store_true",
        help="Combine links when possible, to minimize the clutter. OBS: combines edges of different types.",
    )
...
def main():
    ...
    if args.highlight:
        ...
        if (
            ext_package in net.nodes()
            and net.nodes[ext_package]["_type"] == "ext_package"
        ):
            pass
        else:
            args.highlight = None
            logging.warning(
                f"Cannot find external package to highlight: '{ext_package}'"
            )

    # exclude nodes based on glob pattern args
    excl_nodes = exclude_nodes(
        project_path=path, net=net, excl_pattern=args.excl, incl_pattern=args.incl
    )
    ...
    dot: pydot.Dot = graph_viz_builder.build_dot_layout(
        network=net,
        project_path=path,
        dir_as=args.dir_as,
