...
class EdgeLayout(pydot.Edge):
    ...
    def __init__(self, src: Path, dst: Path, **kwargs) -> None:
        super().__init__(
            src=pydot.Node(src.as_posix()), dst=pydot.Node(dst.as_posix()), **kwargs
        )
...
class DirLayout(pydot.Node):
    ...
    def __init__(self, network: nx.DiGraph, node: Path) -> None:
        super().__init__(name=node.as_posix())
        if _is_package(network=network, node=node):
            self.set("shape", "component")
        else:
            self.set("shape", "folder")
        self.set("color", "red")
...
class FileLayout(pydot.Node):
    ...
    def __init__(
        ...
        filename: str = (
            node.as_posix()
            if full_filepath
            else node.relative_to(node.parent).as_posix()
        )
        filename = filename.replace("/", " / ")

        if with_interface:
            mod_int: ModuleInterface = interface.get_module_interface(module_path=node)
            cs: List[str] = self._class_definition_style(
                class_defs=mod_int.class_defs, show_class_bases=show_class_bases
            )
            vs: List[str] = [v.name for v in mod_int.single_assignments]
            fs: List[str] = self._function_def_style(
                function_defs=mod_int.function_defs,
            ...
            self.set("label", layout)
        else:
            self.set("label", filename)

    def _class_definition_style(
        self, class_defs: List[astroid.ClassDef], show_class_bases: bool
    ) -> List[str]:
