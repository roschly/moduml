...
class GraphVizBuilder:
    def __init__(
        self, network: nx.DiGraph, project_path: Path, rankdir: str = "TB"
    ) -> None:
        ...
        self.internal_import_links = [
            (src, dst)
            for src, dst, _ in filter_links(network, "import")
            if src in self.file_nodes and dst in self.file_nodes
        ]
    ...
    def add_file_nodes(self, with_interface: bool = False) -> None:
        for n in self.file_nodes:
            ...
            if (
                self.network.has_edge(n, ext_package)
                and self.network.edges[n, ext_package]["_type"] == "import"
            ):
                # node_color = "red"
    ...
    def add_dir_clusters(self) -> None:
        for n in self.dir_nodes:
            ...
            cluster_nodes = [
                ...
                if src == n and self.network.nodes[dst]["_type"] == "file"
            ]
...
def build_dot_layout(
    ...
    builder = GraphVizBuilder(
        network=network, project_path=project_path, rankdir=ARGS.rankdir
    )
